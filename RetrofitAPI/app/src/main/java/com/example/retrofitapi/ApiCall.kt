...
class ApiCall {
    /* This function takes a Context and callback function as a parameter,
    which will be called when the API response is received.*/
        ...
        call.enqueue(object : Callback<DataModel> {
            ...
            override fun onResponse(response: Response<DataModel>?, retrofit: Retrofit?) {
                ...
                if(response!!.isSuccess){
                    // If the response is successful, parse the response body to a DataModel object.
                    val jokes: DataModel = response.body() as DataModel

                    // Call the callback function with the DataModel object as a parameter.
                    callback(jokes)
                }
            }
